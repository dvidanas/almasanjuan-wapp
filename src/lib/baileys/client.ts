...
export async function startBaileys(): Promise<void> {
  ...
  async function connect() {
    ...
    const socket = makeWASocket({
      ...
      browser: ["Alma San Juan", "Chrome", "1.0"],
    ...
    socket.ev.on("connection.update", (update) => {
      ...
      if (connection === "open") {
        ...
        // JID format: "5492646XXXXXX:XX@s.whatsapp.net" — extraer solo el número
      ...
      if (connection === "close") {
        ...
        if (!loggedOut) {
          ...
          // Sesión expirada — limpiar archivos de sesión para forzar nuevo QR
    ...
    // Importar handler dinámicamente para evitar dependencias circulares
